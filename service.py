...
from models import Food, EatenFood, Weight
...
class Service:
    ...
    def add_food(self, name: str, calories_per_100_grams: Decimal) -> Food:
        food = Repository().add_food(name, calories_per_100_grams, self.now())
        return food

    def add_eaten_food(self, food: Food, weight_grams: Decimal) -> None:
        # TODO check if food is portion food, then weight is redundant ?
        now = self.now()
        eaten_food = EatenFood(food, weight_grams=weight_grams, added_on=now)
        Repository().add_eaten_food(eaten_food)
    ...
    def get_today_eaten_calories(self) -> Decimal:
        ...
        today_eaten_calories = [eaten_food.calories_eaten for eaten_food in eaten_foods if eaten_food.added_on > today_start_datetime]
